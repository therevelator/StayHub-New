...
const Planning = () => {
  ...
  const generateAiGuide = async () => {
    ...
    try {
      const selectedInterests = Object.entries(interests || {})
        .filter(([, v]) => v)
        .map(([k]) => k);
      const res = await api.post('/ai/guide', { destination, days: aiDays, interests: selectedInterests });
      const guide = res.data?.data;
      setAiGuide(guide);
      // Best-effort: fit the map to the itinerary. A map failure must never
      // reject the whole flow — the guide and markers should still render.
      try {
        const pts = (guide?.days || [])
          .flatMap((d) => d?.places || [])
  ...
  const generateGuidePdf = () => {
    ...
    let y = 20;
    doc.setFontSize(18);
    doc.text(`Travel Guide — ${aiGuide.destination}`, mx, y);
    y += 8;
    doc.setFontSize(10); doc.setTextColor(120);
    doc.text('Generated by StayHub AI', mx, y); doc.setTextColor(0); y += 8;
    (aiGuide.days || []).forEach((day) => {
      if (y > 255) { doc.addPage(); y = 20; }
      doc.setFontSize(14);
      doc.text(`Day ${day.day}: ${day.title || ''}`, mx, y); y += 6;
      if (day.summary) {
        doc.setFontSize(10); doc.setTextColor(90);
        const l = doc.splitTextToSize(day.summary, 180); doc.text(l, mx, y); y += l.length * 5; doc.setTextColor(0);
      }
      (day.places || []).forEach((p) => {
        if (y > 270) { doc.addPage(); y = 20; }
        doc.setFontSize(11); doc.text(`• ${p.name}`, mx + 2, y); y += 5;
        if (p.description) {
          doc.setFontSize(9); doc.setTextColor(110);
          const l = doc.splitTextToSize(p.description, 172); doc.text(l, mx + 6, y); y += l.length * 4.5; doc.setTextColor(0);
        }
      });
      if (day.hotels?.length) {
        if (y > 262) { doc.addPage(); y = 20; }
        doc.setFontSize(10); doc.setTextColor(31, 122, 109);
        doc.text('Where to stay (StayHub):', mx + 2, y); y += 5; doc.setTextColor(60);
        day.hotels.forEach((h) => {
          doc.setFontSize(9);
          doc.text(`- ${h.name}${h.fromPrice ? ` — from $${Math.round(h.fromPrice)}/night` : ''} (${h.distanceKm} km)`, mx + 6, y);
          y += 4.5;
        });
        doc.setTextColor(0);
      }
                ...
                <div className="mb-5 bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Type a destination in the search above, pick the days, and get an AI day-by-day plan on the map.</p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={generateAiGuide} disabled={aiLoadingGuide} className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-60 text-white font-semibold px-4 py-2 rounded-lg">
                        {aiLoadingGuide ? 'Generating…' : 'Generate AI Guide'}
                      </button>
                      {aiGuide && (
                        <button onClick={generateGuidePdf} className="inline-flex items-center gap-2 border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold px-4 py-2 rounded-lg">
                          Download PDF
                        </button>
                      )}
                    </div>
